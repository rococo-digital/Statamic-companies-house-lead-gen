...
<div class="flex items-center mb-6">
    ...
    <h1 class="flex-1">Create New Rule</h1>
</div>
...
<form method="POST" action="{{ cp_route('ch-lead-gen.rules.store') }}" class="publish-form">
    @csrf
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" 
                           class="input-text" placeholder="e.g. 6 Month Old Companies" required>
                    <div class="help-text">Give your rule a descriptive name</div>
                </div>

                <div class="form-group">
                    <label class="block font-medium">Description</label>
                    <textarea name="description" class="input-text" rows="3" 
                              placeholder="Brief description of what this rule does">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="enabled" value="1" 
                               {{ old('enabled', $defaults['enabled'] ?? true) ? 'checked' : '' }} class="mr-2">
                        <span class="font-medium">Enable this rule</span>
                    </label>
                    <div class="help-text">Disabled rules won't run automatically but can be triggered manually</div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="days_ago" value="{{ old('days_ago', $defaults['days_ago'] ?? 180) }}" 
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="max_results" value="{{ old('max_results', $defaults['max_results'] ?? 50) }}" 
                    ...
                    <div class="form-group">
                        ...
                        <select name="company_status" class="input-text" required>
                            <option value="active" {{ old('company_status', $defaults['company_status'] ?? 'active') == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="dissolved" {{ old('company_status', $defaults['company_status'] ?? '') == 'dissolved' ? 'selected' : '' }}>Dissolved</option>
                            <option value="liquidation" {{ old('company_status', $defaults['company_status'] ?? '') == 'liquidation' ? 'selected' : '' }}>Liquidation</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="company_type" class="input-text" required>
                            <option value="ltd" {{ old('company_type', $defaults['company_type'] ?? 'ltd') == 'ltd' ? 'selected' : '' }}>Private Limited Company (LTD)</option>
                            <option value="plc" {{ old('company_type', $defaults['company_type'] ?? '') == 'plc' ? 'selected' : '' }}>Public Limited Company (PLC)</option>
                            <option value="llp" {{ old('company_type', $defaults['company_type'] ?? '') == 'llp' ? 'selected' : '' }}>Limited Liability Partnership (LLP)</option>
                            <option value="partnership" {{ old('company_type', $defaults['company_type'] ?? '') == 'partnership' ? 'selected' : '' }}>Partnership</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="flex flex-wrap gap-4 mt-2">
                        @php $selectedCountries = old('allowed_countries', $defaults['allowed_countries'] ?? ['GB']); @endphp
                        <label class="flex items-center">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="allowed_countries[]" value="IE" 
                                   {{ in_array('IE', $selectedCountries) ? 'checked' : '' }} class="mr-2">
                            <span>Ireland (IE)</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="allowed_countries[]" value="CA" 
                                   {{ in_array('CA', $selectedCountries) ? 'checked' : '' }} class="mr-2">
                            <span>Canada (CA)</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="allowed_countries[]" value="AU" 
                                   {{ in_array('AU', $selectedCountries) ? 'checked' : '' }} class="mr-2">
                            <span>Australia (AU)</span>
                        </label>
                    </div>
                    <div class="help-text">Select which countries to include in the search</div>
                </div>

                <div class="form-group">
                    <label class="flex items-center">
                        <input type="checkbox" name="check_confirmation_statement" value="1" 
                               {{ old('check_confirmation_statement', $defaults['check_confirmation_statement'] ?? false) ? 'checked' : '' }} class="mr-2">
                        <span class="font-medium">Check Confirmation Statements</span>
                    </label>
                    <div class="help-text">Only include companies that have missing confirmation statements (requires additional API calls)</div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="schedule_enabled" value="1" 
                               {{ old('schedule_enabled', $defaults['schedule_enabled'] ?? true) ? 'checked' : '' }} class="mr-2" id="schedule_enabled">
                ...
                <div id="schedule_options" class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                    <div class="form-group">
                        ...
                        <select name="frequency" class="input-text" required>
                            <option value="daily" {{ old('frequency', $defaults['frequency'] ?? 'daily') == 'daily' ? 'selected' : '' }}>Daily</option>
                            <option value="weekly" {{ old('frequency', $defaults['frequency'] ?? '') == 'weekly' ? 'selected' : '' }}>Weekly</option>
                            <option value="monthly" {{ old('frequency', $defaults['frequency'] ?? '') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="block font-medium">Time *</label>
                        <input type="time" name="time" value="{{ old('time', $defaults['time'] ?? '09:00') }}" class="input-text" required>
                    ...
                    <div class="form-group">
                        ...
                        <select name="day_of_month" class="input-text">
                            <option value="">Not applicable</option>
                            @for ($i = 1; $i <= 31; $i++)
                                <option value="{{ $i }}" {{ old('day_of_month', $defaults['day_of_month'] ?? 1) == $i ? 'selected' : '' }}>{{ $i }}</option>
                            @endfor
                        </select>
                ...
                <div class="bg-blue-50 border-l-4 border-blue-400 p-4 mt-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-blue-800">Schedule Tips</h3>
                            <div class="mt-2 text-sm text-blue-700">
                                <ul class="list-disc pl-5 space-y-1">
                                    <li><strong>Daily:</strong> Searches companies from specific day (no duplicates)</li>
                                    <li><strong>Weekly/Monthly:</strong> Searches date ranges for larger batches</li>
                                    <li><strong>Recommended:</strong> Daily for consistent processing, weekly for batching</li>
                                </ul>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="instantly_enabled" value="1" 
                               {{ old('instantly_enabled', $defaults['instantly_enabled'] ?? false) ? 'checked' : '' }} class="mr-2" id="instantly_enabled">
                ...
                <div id="instantly_options" class="space-y-4 mt-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="lead_list_name" value="{{ old('lead_list_name') }}" 
                    ...
                    <div class="form-group">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="enable_enrichment" value="1" 
                                   {{ old('enable_enrichment', $defaults['enable_enrichment'] ?? false) ? 'checked' : '' }} class="mr-2">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="webhook_enabled" value="1" 
                               {{ old('webhook_enabled', $defaults['webhook_enabled'] ?? false) ? 'checked' : '' }} class="mr-2" id="webhook_enabled">
                ...
                <div id="webhook_options" class="space-y-4 mt-4">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="webhook_secret" value="{{ old('webhook_secret') }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleWebhookOptions(); // Set initial state
